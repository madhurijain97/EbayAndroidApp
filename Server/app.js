...
app.get('/searchButton', function(req, res){
	...
	console.log(req.query);
		...
		i += 1;
		if(req.query.conditionNew === "false" && req.query.conditionUsed === "false" && req.query.conditionUnspecified === "false")
...
app.get('/productsTab', function(req, res){
	...
	//var apiCall = "http://open.api.ebay.com/shopping?callname=GetSingleItem&responseencoding=JSON&appid=MadhuriJ-WebTech6-PRD-0a6d2f14d-21b6a625&siteid=0&version=967&ItemID=143204882533&IncludeSelector=Description,Details,ItemSpecifics"
...
app.get('/similarItemsTab', function(req, res){
	...
	//var apiCall = "http://svcs.ebay.com/MerchandisingService?OPERATION-NAME=getSimilarItems&SERVICE-NAME=MerchandisingService&SERVICE-VERSION=1.1.0&CONSUMER-ID=MadhuriJ-WebTech6-PRD-0a6d2f14d-21b6a625&RESPONSE-DATA-FORMAT=JSON&REST-PAYLOAD&itemId=143204882533&maxResults=20";
...
app.get('/photosTab', function(req, res){
	...
	//var apiCall = "https://www.googleapis.com/customsearch/v1?q=Apple%20iPhone%208%20Plus%20-%2064GB%20-%20Space%20Gray%20(T-Mobile)%20A1897%20(GSM)&cx=005104929404727311061:ubjdibpefja&imgSize=huge&imgType=news&num=8&searchType=image&key=AIzaSyDZL7afqcXyL5fkMtQqOj2C6J77EXA5g-k";
	var apiCall = encodeURI("https://www.googleapis.com/customsearch/v1?q=" + (req.query.keyword) + "&cx=" + cx + "&imgSize=huge&imgType=news&num=8&searchType=image&key=" + googleKey);
	console.log(apiCall);
	request(apiCall, function(error, response, body){
		if(!error && response.statusCode == 200){
			var newBody = JSON.parse(body);
